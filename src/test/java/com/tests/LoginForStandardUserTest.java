...
public class LoginForStandardUserTest extends BaseTest {

	List<String> login;

	@FrameworkAnnotation(author = "Poonam")
	@Test(description = "To validate login functionality for standard user")
	public void loginForStandardUserTest() {

		try {
			login = ExcelUtils.readData("loginForStandardUserTest"); // pass test case name as an argument so that data
																		// would be fetched from excel based on test
																		// case name
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		loginPage.performLogin(login.get(0), login.get(1)); // pass column number as index for respective validation
															// from testdata
